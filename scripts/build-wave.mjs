...
function buildSql() {
  const lines = [];
  const auditKinds = [
    audit.filter((c) => c.proposed).length ? `${audit.filter((c) => c.proposed).length} vector` : null,
    audit.filter((c) => c.proposedTags).length ? `${audit.filter((c) => c.proposedTags).length} tag` : null,
  ].filter(Boolean);
  lines.push(`-- Wave ${waveNumber} — ${survivors.length} new activities` +
    (auditKinds.length ? ` and ${auditKinds.join(" + ")} correction(s) to existing rows` : ""));
  ...
  lines.push("commit;");
  lines.push("");
  lines.push("-- Check: paste this after the block above. Both numbers are exact.");
  lines.push("select count(*) as total_activities,");
  lines.push("       count(*) filter (where vector is null) as missing_vector");
  lines.push("from public.activities;");
  lines.push(`-- expect ${seeded.length + survivors.length} rows, 0 missing_vector`);
  if (audit.filter((c) => c.proposedTags).length) {
    lines.push("");
    lines.push("-- And that the tag corrections landed:");
    lines.push("select title, tags from public.activities");
    lines.push(
      "where title in (" +
        audit.filter((c) => c.proposedTags).map((c) => sqlLiteral(c.title)).join(", ") +
        ");"
    );
  }
  lines.push("");
