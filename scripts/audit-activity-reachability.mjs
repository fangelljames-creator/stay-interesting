 ...
 *
 * ⚠️ WHICH REGIME THIS MEASURES. Production on `main` today: hard filters, then
 * graceful relaxation, then rankActivities, then the top 3 plus reroll ranks
 * 4-8. THERE IS NO diverseSelect IN THIS MEASUREMENT, because there is none on
 * main — the greedy diversity re-rank is unmerged on the `result-diversity`
 * branch. When it lands, this audit has to be re-run: a pass that skips
 * near-duplicates will pull DIFFERENT rows into the earned-8, and some of what
 * is dark here may stop being dark. The header of the generated report says so
 * too, so a stale copy cannot be mistaken for a current one.
 *
...
push();
push(
  "⚠️ **This measures the pipeline as it stands on `main`:** hard filters → graceful relaxation →"
);
push(
  "`rankActivities` → the top 3 plus reroll ranks 4–8. **There is no `diverseSelect` in these"
);
push(
  "numbers**, because there is none on `main` — the greedy diversity re-rank is unmerged on the"
);
push(
  "`result-diversity` branch. When it merges, re-run this: a pass that skips near-duplicates pulls"
);
push("different rows into the earned-8, and some of what is dark here may stop being dark.");
push();
...
console.log(`\n    Wrote ${outPath}`);
console.log(
  `\n    ⚠️  Measured WITHOUT diverseSelect — that is the regime on main. Re-run after\n` +
    `        result-diversity merges, and after every content wave.`
);
